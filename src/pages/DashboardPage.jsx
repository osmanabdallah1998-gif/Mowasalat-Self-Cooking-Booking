...
function OccupancyRow({ loc, onClick }) {
  ...
  return (
    <tr className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => onClick(loc.id)}>
      ...
      <td className="px-4 py-3 text-sm text-gray-600 text-right">{loc.occupancy.toLocaleString()} / {loc.capacity.toLocaleString()}</td>
      ...
      <td className="px-4 py-3 text-sm text-right">
        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${pct >= 95 ? 'bg-red-100 text-red-700' : pct >= 80 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
          {pct}%
        </span>
      </td>
    </tr>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Capacity"   value={totalCap.toLocaleString()}  sub="Staff beds"        icon={Building2}   accent="bg-[#005F9E]" />
        <StatCard label="Currently Occupied" value={totalOcc.toLocaleString()} sub={`${Math.round((totalOcc/totalCap)*100)}% utilisation`} icon={Users} accent="bg-[#8CC63F]" />
        <StatCard label="Available Beds"   value={available.toLocaleString()} sub="Across all depots" icon={TrendingUp}   accent="bg-purple-500" />
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-4 border-b">
          ...
          <button onClick={() => navigate('/locations')} className="text-sm text-[#005F9E] hover:underline flex items-center gap-1">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-4 border-b">
          ...
          <button onClick={() => navigate('/maintenance')} className="text-sm text-[#005F9E] hover:underline flex items-center gap-1">
            View all <ChevronRight size={14} />
          </button>
        </div>
