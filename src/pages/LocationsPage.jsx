...
function LocationCard({ data, onClick }) {
  ...
  const badgeColor = pct >= 95 ? 'bg-red-500' : pct >= 80 ? 'bg-amber-500' : 'bg-[#8CC63F]';

  return (
    <div onClick={() => onClick(data.id)}
      className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all cursor-pointer border-l-4 border-l-[#8CC63F] group"
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center text-gray-500 text-sm mb-3">
          <MapPin size={14} className="mr-1 shrink-0" />{data.location}
        </div>
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{filtered.length} accommodation{filtered.length !== 1 ? 's' : ''}</p>
        </div>
            ...
            {['All', 'Staff', 'Admin'].map(t => (
              <button key={t} onClick={() => setFilter(t)}
                className={`px-5 py-2 rounded-md text-sm font-medium transition-colors ${
                  filter === t ? 'bg-[#8CC63F] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
                }`}>
                {t}
              </button>
            ))}
