...
export default function TenantsPage() {
  ...
  const filtered = tenants.filter(t => {
    ...
    const matchSearch = !q || t.name.toLowerCase().includes(q) || t.employeeId.toLowerCase().includes(q) || t.room.toLowerCase().includes(q);
    const matchLoc    = locFilter === 'all' || t.locationId === locFilter;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{filtered.length} record{filtered.length !== 1 ? 's' : ''} shown</p>
        </div>
        <button
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} className="text-center py-12 text-sm text-gray-400">
                    No tenants found. Adjust your filters or add a new tenant.
                  </td>
                </tr>
              ) : (
                filtered.map(t => (
                  <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-[#005F9E] flex items-center justify-center text-white text-xs font-bold shrink-0">
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className={`text-xs font-semibold rounded-full px-2 py-0.5 border-0 cursor-pointer focus:outline-none ${STATUS_COLORS[t.status]}`}
                      >
                        <option value="Active">Active</option>
                        <option value="Inactive">Inactive</option>
                      </select>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input required type="text" value={form.employeeId}
              ...
              <div>
                ...
                <input required type="text" value={form.room}
      ...
      {deleteId && (
        <Modal title="Remove Tenant" onClose={() => setDeleteId(null)}>
          <p className="text-sm text-gray-600 mb-6">
            Are you sure you want to remove <strong>{tenants.find(t => t.id === deleteId)?.name}</strong>? This action cannot be undone.
          </p>
          <div className="flex gap-3">
            ...
            <button onClick={() => { removeTenant(deleteId); setDeleteId(null); }}
